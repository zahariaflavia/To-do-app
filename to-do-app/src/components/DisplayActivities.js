        ...
        const DisplayActivities=()=>{
            ...
            return <div className="items">
                ...
                {(aux.length!==0 && aux.filter((task)=>task.status==='incomplete').length===0)?(<h2>Congratulations! You finished all your activities for today!</h2>):(<h2>Activities to be completed today: {aux.filter((task)=>task.status==='incomplete').length}</h2>)}
                </div>
